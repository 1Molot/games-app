...
export const Filters = ({ results, setFilteredGames }: FiltersProps) => {
  ...
  useEffect(() => {
    if (results) {
      ...
      if (platformFilter) {
        filtered = filtered.filter((game) => game.parent_platforms.some((p) => p.platform.name === platformFilter));
      }

      if (languageFilter) {
        filtered = filtered.filter((game) => game.tags.some((tag) => tags[tag.language] === languageFilter));
      }

      if (playersFilter) {
        filtered = filtered.filter(
          (game) => (game.tags.find((tag) => tag.slug === 'multiplayer')?.games_count || 0) < playersFilter
        );
      }
